...
namespace AsyncConverter.Helpers
{
    ...
    public class AsyncReplacer : IAsyncReplacer
    {
        ...
        {
            foreach (var declaration in method.FindAllHierarchy().SelectMany(x => x.GetDeclarations<IMethodDeclaration>()))
            {
                ReplaceMethodToAsync(declaration);
            }

            foreach (var declaration in method.FindAllHierarchy().SelectMany(x => x.GetDeclarations<ILocalFunctionDeclaration>()))
            {
                ReplaceMethodToAsync(declaration);
            }
        }

        public void ReplaceToAsync(ILocalFunction method)
        {
            foreach (var declaration in method.GetDeclarations<IMethodDeclaration>())
            {
                ReplaceMethodToAsync(declaration);
            }

            foreach (var declaration in method.GetDeclarations<ILocalFunctionDeclaration>())
            {
                ReplaceMethodToAsync(declaration);
            }
        }
        ...
        private void ReplaceMethodToAsync(ILocalFunctionDeclaration method)
        {
            if (!method.IsValid())
                return;
        ...
        private void ReplaceMethodSignatureToAsync([NotNull] IParametersOwner parametersOwner, [NotNull] ILocalFunctionDeclaration methodDeclaration)
        {
            var returnType = parametersOwner.ReturnType;
        ...
        private static void SetSignature([NotNull] ILocalFunctionDeclaration methodDeclaration, [NotNull] IType newReturnValue, [NotNull] string newName)
        {
            methodDeclaration.SetType(newReturnValue);
            if (!methodDeclaration.IsAbstract)
                methodDeclaration.SetAsync(true);
